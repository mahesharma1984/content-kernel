...
Changes from v0.3:
- Editorial visual identity (Playfair Display serif, warm palette)
- Visual logic enhancements:
  - Layer 1: Three pillars with icons
  - Layer 2: Freytag arc diagram
  - Layer 3: Convergence diagram (inputs → synthesis)
  - Layer 4: Component stack assembly
...
<body class="bg-cream min-h-screen font-sans text-ink">
    <div class="max-w-lg mx-auto bg-white min-h-screen shadow-sm border-x border-rule">
        ...
        <header class="px-6 pt-8 pb-6 border-b border-rule">
            <a href="/" class="text-xs tracking-widest text-inkMuted uppercase hover:text-accent transition-colors">&larr; All Texts</a>
            ...
            <div id="meaning-content" class="accordion-content px-6 text-ink">
                <div class="pb-6">
                    ...
                    <div class="mb-6 flex justify-center">
                        <svg width="320" height="70" viewBox="0 0 320 70" class="text-accent">
                            <path d="M20 60 Q80 55 120 35 Q160 10 160 10 Q160 10 200 35 Q240 55 300 60" stroke="currentColor" stroke-width="2" fill="none"/>
                            <circle cx="40" cy="58" r="4" fill="currentColor" opacity="0.5"/>
                            <circle cx="100" cy="42" r="4" fill="currentColor" opacity="0.5"/>
                            <circle cx="160" cy="12" r="5" fill="currentColor"/>
                            <circle cx="220" cy="42" r="4" fill="currentColor" opacity="0.5"/>
                            <circle cx="280" cy="58" r="4" fill="currentColor" opacity="0.5"/>
                            <text x="40" y="70" text-anchor="middle" class="text-[7px] fill-inkMuted uppercase">Expo</text>
                            <text x="100" y="54" text-anchor="middle" class="text-[7px] fill-inkMuted uppercase">Rising</text>
                            <text x="160" y="8" text-anchor="middle" class="text-[7px] fill-accent font-medium uppercase">Climax</text>
                            <text x="220" y="54" text-anchor="middle" class="text-[7px] fill-inkMuted uppercase">Falling</text>
                            <text x="280" y="70" text-anchor="middle" class="text-[7px] fill-inkMuted uppercase">Reso</text>
                        </svg>
                    </div>
        ...
        <div class="border-b border-rule">
            <button onclick="toggleAccordion('devices')" class="w-full py-6 px-6 flex items-center justify-between text-left hover:bg-cream/50 transition-colors">
                <span class="font-serif text-xl font-medium text-ink">How it's achieved</span>
                <svg id="devices-chevron" class="chevron w-6 h-6 text-inkMuted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
        ...
        <section class="px-6 py-6 border-t border-rule">
            <a href="#workbook" class="block bg-accentLight rounded p-5 flex items-center justify-between hover:bg-accentLight/70 transition-colors group">
                <div>
                    <p class="font-serif font-medium text-ink group-hover:text-accent transition-colors">Learn to write this yourself</p>
                    <p class="text-sm text-accent mt-1">{title} Essay Workbook &rarr;</p>
                </div>
        ...
        <footer class="px-6 py-6 border-t border-rule">
            <p class="text-xs text-inkMuted text-center"><a href="/" class="hover:text-accent transition-colors">&larr; All texts</a> &middot; &copy; 2024</p>
        </footer>
    </div>
...
DEVICE_TEMPLATE = '''<div class="p-5 bg-cream rounded-lg border-l-4 border-accent">
    <div class="flex justify-between items-start mb-3">
        <p class="text-sm tracking-wide text-accent uppercase font-semibold">{name}</p>
        <div class="flex items-center gap-3">
            <span class="text-sm text-inkMuted bg-rule px-3 py-1 rounded">{section_label}</span>
            <span class="text-sm text-inkMuted">Ch. {chapter}</span>
        </div>
    </div>
    <p class="font-serif text-lg text-ink italic mb-4 leading-relaxed">"{quote}"</p>
    <p class="text-base text-inkLight leading-relaxed">{effect}</p>
</div>'''
...
def generate_devices_html(devices: list) -> str:
    ...
    for device in devices:
        section_label = SECTION_LABELS.get(device.get('section', ''), 'Unknown')
